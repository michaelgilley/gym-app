...
Single-file PWA. Everything is in `index.html`: dark CSS in the main `<style>`, all light-theme overrides in a second `<style id="lightTheme">` at the end of `<head>`, HTML `<body>`, JS in one `<script>`. Supporting files: `sw.js`, `manifest.json`, icons. Tests are Playwright in `tests/` (one project, `mobile-safari` / webkit; served via `python3 -m http.server` on `GYM_APP_PORT`, default 8765).

* **Theming:** dark is the base; every light-mode rule lives in `#lightTheme`, whose `media` attribute is the theme switch (`(prefers-color-scheme: light)` = system, `all` = forced light, `not all` = forced dark). The `#themeToggle` button cycles system → light → dark via `cycleThemeMode()` (boot script in `<head>`, so there's no flash and no-JS still follows the OS). New light-mode CSS goes in that sheet, not in a `@media (prefers-color-scheme: light)` block — and remember it now sits *after* everything else, so a rule that was previously outranked by a later same-specificity rule will start winning.
